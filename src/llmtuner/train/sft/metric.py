...
def O_Knowledge_R_P_F1(hyps, refs, cues):
    """
    optimized Knowledge_R_P_F1
    ...
    P_list = []
    F1_list = []
    for hyp, ref, cue in zip(hyps, refs, cues):
        ...
        if len(cue_counter) == 0:
            continue
        hyp_counter -= sw_counter
        ref_counter -= sw_counter

        r = (cue_counter & ref_counter & hyp_counter).__len__() / (cue_counter.__len__() + 0.00001)
        R_list.append(r)

        p = (cue_counter & ref_counter & hyp_counter).__len__() / (hyp_counter.__len__() + 0.00001)
        P_list.append(p)
...
class ComputeMetrics:
    ...
    def __call__(self, eval_preds: Sequence[Union[np.ndarray, Tuple[np.ndarray]]]) -> Dict[str, float]:
        ...
        #     data = json.load(file)
        with open('data/a_testexp.json', 'r') as file:
            data = json.load(file)
